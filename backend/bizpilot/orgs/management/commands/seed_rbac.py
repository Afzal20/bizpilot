...
class Command(BaseCommand):
    help = "Seed RBAC v2 permissions catalog and default roles for existing organizations."
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Successfully seeded permissions. Total catalog permissions: {total_perms}"
            )
        )

        orgs = Organization.objects.all()
        self.stdout.write(f"Seeding default system roles for {orgs.count()} organizations...")
        for org in orgs:
